<script lang="ts">
    import { ArrowLeft, Save, Plus, Trash, Settings, Users, Check, X, ChevronDown, ChevronLeft, ChevronRight, Tag, Monitor, RefreshCw } from "lucide-svelte";
    ...
    // MQTT handling via composable (useDeviceProfileMqtt)
    const { setup: setupProfileMqtt, cleanup: cleanupProfileMqtt } = useDeviceProfileMqtt({
        profileId,
        onStatusUpdate: loadAssignedDevices,
        onProgressUpdate: loadAssignedDevices,
    ...
    // Function to subscribe to device events (no longer needed with MQTT - kept for compatibility)
    async function subscribeToDeviceEvents(deviceIds: string[]) {
        // MQTT subscriptions are automatic via device-specific topics
        // No explicit subscription needed
        console.log(`[DeviceAssignmentManager] MQTT auto-subscribed to ${deviceIds.length} devices`);
    }
    ...
    onMount(async () => {
        ...
        // Setup MQTT listeners via composable
        setupProfileMqtt();
    });

    // Clean up event listeners on component destroy
    onDestroy(() => {
        console.log('[DeviceAssignmentManager] Cleaning up MQTT listeners');
        cleanupProfileMqtt();
    });

    // Reactive statement to re-setup event listeners when connId changes
    $: if (connId) {
        // The event listeners are already set up in onMount, but we can add additional logic here if needed
    }
    ...
    async function loadAssignedDevices() {
        try {
            ...
            const apiPath = `/api/device-profiles/${profileId}/devices`;
    ...
    async function handleAssign() {
        ...
        try {
            ...
            const apiPath = `/api/device-profiles/${profileId}/assign`;
    ...
    // Handle reapply all devices
    async function handleReapplyAll() {
        if (assignedDevices.length === 0) return;
        
        deviceLoading = true;
        try {
            const deviceIds = assignedDevices.map(device => device.id).filter(Boolean);
            
            // Unified endpoint - works for both admin and user
            const apiPath = `/api/device-profiles/${profileId}/reapply`;
            ...
            if (!response.ok) throw new Error('Failed to reapply profile');

            const result = await response.json();
            const message = result.message || `Profile reapplied to ${deviceIds.length} device(s)`;
            toast.success(message);
            
            // Refresh the assigned devices to show updated status
            await loadAssignedDevices();
        } catch (error) {
            console.error('Error reapplying profile:', error);
            toast.error('An error occurred while reapplying the profile');
        } finally {
            deviceLoading = false;
        }
    }

    // Handle reapply single device
    async function handleReapplySingle(deviceId: string) {
        deviceLoading = true;
        try {
            // Unified endpoint - works for both admin and user
            const apiPath = `/api/device-profiles/${profileId}/reapply`;
            ...
            toast.success('Profile reapplied to device');
    ...
    async function handleUnassign(deviceId: string) {
        try {
            const apiPath = `/api/device-profiles/${profileId}/unassign`;
    ...
    async function handleUnassignAll() {
        ...
        deviceLoading = true;
        try {
            const apiPath = `/api/device-profiles/${profileId}/unassign-all`;
    ...
    async function handleAddAll() {
        ...
        try {
            ...
            // Device subscriptions now handled via MQTT - no manual subscription needed
            // MQTT automatically handles device notifications based on user permissions

            const apiPath = `/api/device-profiles/${profileId}/assign-all`;
    ...
    async function loadAllAvailableTags() {
        try {
            const apiPath = isAdmin ? `/api/admin/iot/devices/tags` : `/api/user/iot/devices/tags`;
            const response = await fetch(apiPath);
            const result = await response.json();
            if (result.success) {
    ...
    async function loadTagDevices(tagId: string) {
        ...
        try {
            const apiPath = `/api/device-profiles/${profileId}/devices-by-tag`;
            const response = await fetch(`${apiPath}?tagId=${tagId}`);
            const result = await response.json();
            if (result.success) {
    ...
    async function handleAssignByTag() {
        ...
        try {
            const apiPath = `/api/device-profiles/${profileId}/assign-by-tag`;
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card class="border-l-4 border-l-purple-500 bg-gradient-to-r from-purple-50/50 to-transparent">
            <CardContent class="p-4">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="text-sm font-medium text-muted-foreground">Profile Status</p>
                        <div class="flex items-center gap-2 mt-1">
                            {#if assignedDevices.length > 0}
                                {@const appliedCount = assignedDevices.filter(d => d.profileAssignment?.status === 'SUCCESS').length}
                                {@const applyingCount = assignedDevices.filter(d => d.profileAssignment?.status === 'APPLYING').length}
                                {@const failedCount = assignedDevices.filter(d => d.profileAssignment?.status === 'FAILED').length}
                                {@const activeCount = assignedDevices.filter(d => d.profileAssignment?.status === 'ACTIVE').length}
                                
                                {#if appliedCount > 0}
                                    <Badge variant="default" class="bg-green-600 text-xs">{appliedCount} Applied</Badge>
                                {/if}
                                {#if applyingCount > 0}
                                    <Badge variant="default" class="bg-yellow-600 text-xs">{applyingCount} Applying</Badge>
                                {/if}
                                {#if failedCount > 0}
                                    <Badge variant="destructive" class="text-xs">{failedCount} Failed</Badge>
                                {/if}
                                {#if activeCount > 0}
                                    <Badge variant="default" class="bg-blue-600 text-xs">{activeCount} Active</Badge>
                                {/if}
                                {#if appliedCount === 0 && applyingCount === 0 && failedCount === 0 && activeCount === 0}
                                    <span class="text-xs text-gray-500">No status data</span>
                                {/if}
                            {:else}
    ...
    {#if assignedDevices.length > 0}
        <div class="flex justify-end mb-4">
            <Button 
                variant="outline" 
                size="sm" 
                on:click={handleReapplyAll}
                disabled={deviceLoading}
                class="flex items-center gap-2"
            >
                <RefreshCw class="h-4 w-4" />
                Reapply All ({assignedDevices.length} devices)
            </Button>
        </div>
    {/if}
                                ...
                                <div class="flex items-center gap-2 min-w-0 flex-1">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <div class="text-xs text-green-700">
                                            {getDeviceTypeDisplayName(device.deviceType)} • {device.status} • MAC: {device.macAddress || 'N/A'}
                                        </div>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2 flex-shrink-0">
                                    <!-- Enhanced Status Badge -->
                                    {#if device.profileAssignment.status === 'SUCCESS'}
                                        <Badge variant="default" class="bg-green-600 hover:bg-green-700 text-xs font-medium">
                                            <Check class="h-3 w-3 mr-1" />
                                            Applied
                                        </Badge>
                                    {:else if device.profileAssignment.status === 'APPLYING'}
                                        <Badge variant="default" class="bg-yellow-600 hover:bg-yellow-700 text-xs font-medium">
                                            <Settings class="h-3 w-3 mr-1 animate-spin" />
                                            Applying
                                        </Badge>
                                    {:else if device.profileAssignment.status === 'FAILED'}
                                    ...
                                    <!-- Application Details Tooltip -->
                                    {#if device.profileAssignment.appliedAt}
                                        <div class="relative group">
                                            <div class="w-2 h-2 bg-blue-500 rounded-full cursor-help" title="Click for details"></div>
                                            <div class="absolute right-0 top-6 w-64 p-3 bg-white border rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200 z-10">
                                                <div class="text-xs space-y-1">
                                                    {#if device.profileAssignment.appliedAt}
                                                        <div class="text-gray-600">
                                                            <strong>Applied:</strong> {new Date(device.profileAssignment.appliedAt).toLocaleString()}
                                                        </div>
                                                    {/if}
                                                    {#if device.profileAssignment.assignedAt}
                                                        <div class="text-gray-600">
                                                            <strong>Assigned:</strong> {new Date(device.profileAssignment.assignedAt).toLocaleString()}
                                                        </div>
                                                    {/if}
                                                    {#if device.profileAssignment.lastSyncAt}
                                                        <div class="text-gray-600">
                                                            <strong>Last Sync:</strong> {new Date(device.profileAssignment.lastSyncAt).toLocaleString()}
                                                        </div>
                                                    {/if}
                                                </div>
                                            </div>
                                    ...
                                    <Button 
                                        variant="outline" 
                                        size="sm"
                                        on:click={() => handleReapplySingle(device.id)}
                                        disabled={deviceLoading}
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl mx-4 h-[90vh] overflow-hidden flex flex-col">
            ...
            <div class="p-6 space-y-4 flex-1 overflow-y-auto">
                <div>
                    <label for="tag-select" class="block text-sm font-medium mb-2">Select Tag</label>
                    <SearchableTagSelect
                        value={selectedTagId}
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-muted-foreground">
                                                    {getDeviceTypeDisplayName(device.deviceType)} • {device.macAddress || 'No MAC'}
                                                </p>
                                            </div>
                                        </div>
