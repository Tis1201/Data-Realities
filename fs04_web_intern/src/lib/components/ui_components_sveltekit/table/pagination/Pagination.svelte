<script lang="ts">
    ...
    export let emitOnly: boolean = false;
    const dispatch = createEventDispatcher<{ change: { page: number; per_page: number } }>();
    ...
    async function goToPage(page: number) {
        if (emitOnly) {
            dispatch('change', { page, per_page: pagination.per_page });
            return;
        }
        const url = new URL(window.location.href);
        url.searchParams.set('page', page.toString());
        await goto(`?${url.searchParams.toString()}`, { keepFocus: true, noScroll: true });
    }

    // Handle per page change
    async function handlePerPageChange(newPerPage: number) {
        if (emitOnly) {
            dispatch('change', { page: 1, per_page: newPerPage });
            return;
        }
        const url = new URL(window.location.href);
