<script lang="ts">
    ...
    /** Template data to edit */
    export let template: TemplateDataInternal | null = null;
    ...
    /** Incoming alert settings (e.g. from server) with optional fields; normalized to AlertState in fillForm. */
    interface AlertSettingsInput {
    ...
    interface TemplateConfig {
        ...
        alertSettings?: AlertState | AlertSettingsInput;
    }
    interface TemplateDataInternal {
        id: string;
        name: string;
        type: 'Alert' | 'Configuration';
        description?: string | null;
        config?: TemplateConfig | null;
        assignedSensors?: SelectedSensor[];
    }
    ...
    $: alertZoneOptions = (template?.type === 'Alert' ? (zones.length > 0 ? zones : [{ id: 'zone-1', name: 'Zone 1' }]) : zones).map((z) => ({ id: z.id ?? 'zone-1', label: z.name || z.id || 'Zone 1' }));
    ...
    // Tabs depend on template type: Alert template → Alert + Assign Sensor; Configuration → Configuration + Assign Sensor
    const CONFIG_TABS = [
    ...
    $: TABS = template?.type === 'Alert' ? ALERT_TABS : CONFIG_TABS;
    ...
    function fillForm() {
        ...
        activeTab = template.type === 'Alert' ? 'alert' : 'configuration';

        // Alert settings (when type === 'Alert'); normalize optional fields from server to full AlertState
        const alert = template.config?.alertSettings;
        if (template.type === 'Alert' && alert) {
            const so = alert.sensorOffline;
            const nd = alert.noData;
            const dt = alert.dwellTime;
            alertState = {
                sensorOffline: {
                    enabled: so?.enabled ?? false,
                    threshold: so?.threshold ?? '5',
                    unit: (so?.unit as AlertThresholdUnit) ?? 'minutes'
                },
                noData: {
                    enabled: nd?.enabled ?? false,
                    threshold: nd?.threshold ?? '30',
                    unit: (nd?.unit as AlertThresholdUnit) ?? 'minutes'
                },
                dwellTime: {
                    enabled: dt?.enabled ?? false,
                    zoneId: dt?.zoneId ?? '',
                    threshold: dt?.threshold ?? '120'
                },
                email: { enabled: alert.email?.enabled ?? false, address: alert.email?.address ?? '' },
                webhook: { enabled: alert.webhook?.enabled ?? false, url: alert.webhook?.url ?? '' }
            };
        ...
        // Tracking Area
        const ta = template.config?.trackingArea;
        if (ta) {
            xMin = String(ta.xMin ?? ta.startX ?? '');
            xMax = String(ta.xMax ?? ta.endX ?? '');
            yMin = String(ta.yMin ?? ta.startY ?? '');
            yMax = String(ta.yMax ?? ta.endY ?? '');
        } else {
            xMin = '';
            xMax = '';
            yMin = '';
        ...
        // Zones - preserve all position data
        const configZones = template.config?.zones;
        if (configZones && configZones.length > 0) {
            zones = configZones.map((z, i) => ({
        ...
        // Device Settings
        const ds = template.config?.deviceSettings;
        if (ds) {
            deviceMode = ds.deviceMode || '';
            timezone = ds.timezone || '';
            pathTracking = ds.pathTracking ?? false;
            dwellThreshold = ds.dwellThreshold ?? 0;
        } else {
            deviceMode = '';
            timezone = '';
            pathTracking = false;
        ...
        // Assigned Sensors
        selectedSensors = template.assignedSensors || [];
    ...
    // Watch for modal open to fill form with template data
    $: if (open && template) {
        fillForm();
    }
    ...
    function validateForm(): boolean {
        ...
        // Validate tracking area only for Configuration type
        const trackingAreaValid = template?.type === 'Alert' ? true : validateTrackingArea();
    ...
    function handleSave() {
        ...
        const payload = {
            ...
            name: templateName,
            description,
            type: template?.type?.toLowerCase() ?? 'configuration',
            ...
            ...(template?.type === 'Alert' && { alertSettings: alertState }),
            selectedSensors
        };
...
>
    <div class="edit-template-modal-body">
        <div class="add-device-fields">
            ...
            {#if activeTab === 'configuration'}
                ...
                <div class="add-device-section">
                    ...
                    <div class="add-device-dwell-row">
                        <label class="add-device-dwell-label" for="edit-template-dwell-slider">Dwell Threshold</label>
            ...
            {:else if activeTab === 'alert' && template?.type === 'Alert'}
                <!-- Alert Tab (same structure as Edit Device -> Alert) -->
                    ...
                    <div class="edit-device-alert-table-wrap">
                        <div class="edit-device-alert-card">
                            <div class="edit-device-alert-table-row">
                                ...
                                <Toggle size="sm" checked={alertState.sensorOffline.enabled} on:change={() => { alertState.sensorOffline.enabled = !alertState.sensorOffline.enabled; alertState = alertState; }} />
                            </div>
                            {#if alertState.sensorOffline.enabled}
                                ...
                                <div class="edit-device-alert-table-row edit-device-alert-input-row edit-device-alert-threshold-row">
                                    ...
                                    <div class="edit-device-alert-field-box">
                                        <InputField label="" type="text" bind:value={alertState.sensorOffline.threshold} placeholder="Enter" />
                        ...
                        <div class="edit-device-alert-card">
                            <div class="edit-device-alert-table-row">
                                ...
                                <Toggle size="sm" checked={alertState.noData.enabled} on:change={() => { alertState.noData.enabled = !alertState.noData.enabled; alertState = alertState; }} />
                            </div>
                            {#if alertState.noData.enabled}
                                ...
                                <div class="edit-device-alert-table-row edit-device-alert-input-row edit-device-alert-threshold-row">
                                    <span class="edit-device-alert-threshold-label">Threshold</span>
                                    <div class="edit-device-alert-field-box">
                                        <InputField label="" type="text" bind:value={alertState.noData.threshold} placeholder="Enter" />
                                        <div class="edit-device-alert-unit-dropdown">
                                            <Dropdown options={alertThresholdUnitOptions} value={alertState.noData.unit} on:change={(e) => { const v = e.detail; alertState.noData.unit = (Array.isArray(v) ? v[0] : v) === 'hours' ? 'hours' : 'minutes'; alertState = alertState; }} width="100%" clearable={false} />
                                        </div>
                                    </div>
                                </div>
                            {/if}
                        ...
                        <div class="edit-device-alert-card">
                            <div class="edit-device-alert-table-row">
                                ...
                                <Toggle size="sm" checked={alertState.dwellTime.enabled} on:change={() => { alertState.dwellTime.enabled = !alertState.dwellTime.enabled; alertState = alertState; }} />
                            </div>
                                ...
                                <div class="edit-device-alert-table-row edit-device-alert-input-row edit-device-alert-label-field-row">
                                    ...
                                    <div class="edit-device-alert-dwell-field">
                                        <div class="edit-device-alert-dwell-zone-wrap">
                                            <Dropdown label="" options={alertZoneOptions} value={alertState.dwellTime.zoneId} on:change={(e) => { const v = e.detail; alertState.dwellTime.zoneId = Array.isArray(v) ? (v[0] ?? '') : (v ?? ''); alertState = alertState; }} width="100%" clearable={false} placeholder="Select zone" />
                                        </div>
                                        <div class="edit-device-alert-dwell-input-wrap">
                                            <InputField label="" type="text" bind:value={alertState.dwellTime.threshold} placeholder="Enter" />
                                        </div>
                    ...
                    <div class="edit-device-alert-table-wrap">
                        <div class="edit-device-alert-card">
                            <div class="edit-device-alert-table-row">
                                ...
                                <Toggle size="sm" checked={alertState.email.enabled} on:change={() => { alertState.email.enabled = !alertState.email.enabled; alertState = alertState; }} />
                            </div>
                            {#if alertState.email.enabled}
                                ...
                                <div class="edit-device-alert-table-row edit-device-alert-input-row edit-device-alert-label-field-row edit-device-alert-field-row-end">
                                    ...
                                    <InputField label="" type="email" bind:value={alertState.email.address} placeholder="Enter email" />
                                </div>
                            {/if}
                        </div>
                        <div class="edit-device-alert-card">
                            <div class="edit-device-alert-table-row">
                                ...
                                <Toggle size="sm" checked={alertState.webhook.enabled} on:change={() => { alertState.webhook.enabled = !alertState.webhook.enabled; alertState = alertState; }} />
                            </div>
                            {#if alertState.webhook.enabled}
                                <div class="edit-device-alert-card-divider"></div>
                                <div class="edit-device-alert-table-row edit-device-alert-input-row edit-device-alert-label-field-row edit-device-alert-field-row-end">
                                    <span class="edit-device-alert-threshold-label">Webhook Address</span>
                                    <InputField label="" type="url" bind:value={alertState.webhook.url} placeholder="https://" />
                                </div>
                            {/if}
                        </div>
                    </div>
